...
def encryption(string, num1, num2):
    res = ''
    for x in string:
        if 97 <= ord(x) <= 122:
            y = (secretKey.index(x.upper()) * num1 + num2) % 26
            res += secretKey[y]
        else:
            res += x
    return res
...
def decryption(string, num1, num2):
    res = ''
    for x in string:
        if 65 <= ord(x) <= 90:
            y = ((secretKey.index(x) - num2) * num1) % 26
...
if __name__ == '__main__':
    ...
    while not over:
        ...
        key1 = input('从中选择出仿射加密用到的密钥：')
        key2 = input('再输入仿射加密用到的一个密钥：')
        a, b = ex_gcd(int(key1), 26)
        if b != 1:
            print(f'输入的\"{key}\"没有逆元！')
        message = input('请输入明文：')
        ciphertext = encryption(message, int(key1), int(key2))
        print(f'加密后的密文是：{ciphertext}')
        flag = input('是否需要解密？（Y：解密；N：不解密）')
        if flag == 'Y' or flag == 'y':
            plaintext = decryption(ciphertext, a, int(key2))
