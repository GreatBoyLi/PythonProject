...
def batch_norm(X, gamma, beta, moving_mean, moving_var, eps, momentum):
    # 通过is_grad_enabled来判断当前模式是训练还是预测模式
    if not torch.is_grad_enabled():
        # 如果是在预测模式下，直接使用传入的移动平均的均值和方差
...
class BatchNorm(nn.Module):
    # num_features：完全连接层输出数量或卷积层的输出通道数。
    # num_dims：2表示完全连接层，4表示头和层
    ...
    def forward(self, X):
        # 如果X还在内存上，将moving_mean和moving_var复制到X所在的显存上
...
net = torch.nn.Sequential(
    li.Reshape(), nn.Conv2d(1, 6, kernel_size=5, padding=2), BatchNorm(6, num_dims=4),
    nn.Sigmoid(),
