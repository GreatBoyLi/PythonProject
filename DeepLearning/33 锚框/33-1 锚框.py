...
def multibox_prior(data, sizes, ratios):
    ...
    # 除以2来获得半高和半宽
    a = torch.stack((-w, -h, w, h))
    b = a.T
    anchor_manupulations = b.repeat(in_height * in_width, 1) / 2

    # 每个中心点都将有“boxes_per_pixel”个锚框，所以生成所有锚框中心的网格，重复了“boxes_per_pixel”次
    c = torch.stack([shift_x, shift_y, shift_x, shift_y], dim=1)
    out_grid = c.repeat_interleave(boxes_per_pixel, dim=0)
...
def show_bboxes(axes, bboxes, labels=None, colors=None):
    ...
    for i, bbox in enumerate(bboxes):
        ...
        rect = li.bbox_to_rect(bbox.detach().numpy(), color)
        axes.add_patch(rect)
...
def offset_boxes(anchors, assigned_bb, eps=1e-6):
    """对锚框偏移量的转换"""
    c_anc = li.box_corner_to_center(anchors)
    c_assigned_bb = li.box_corner_to_center(assigned_bb)
...
def offset_inverse(anchors, offset_preds):
    """根据带有预测偏移量的锚框来预测边界框"""
    anc = li.box_corner_to_center(anchors)
    pred_bbox_xy = (offset_preds[:, :2] * anc[:, 2:] / 10) + anc[:, :2]
    pred_bbox_wh = torch.exp(offset_preds[:, 2:] / 5) * anc[:, 2:]
    pred_bbox = torch.cat((pred_bbox_xy, pred_bbox_wh), axis=1)
    predicted_bbox = li.box_center_to_corner(pred_bbox)
