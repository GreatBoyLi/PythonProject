...
def blk_forward(X, blk, size, ratio, cls_predictor, bbox_predictor):
    Y = blk(X)
    anchors = li.multibox_prior(Y, sizes=size, ratios=ratio)
...
def cls_eval(cls_preds, cls_labels):
    a = cls_preds.argmax(dim=-1)
    b = a.type(cls_labels.dtype)
...
def predict(net, X):
    ...
    output = li.multibox_detection(cls_probs, bbox_preds, anchors)
...
def display(img, output, threshold):
    ...
    for row in output:
        ...
        li.show_bboxes(fig.axes, bbox, '%.2f' % score, 'w')
    plt.show()
...
batch_size = 32
train_iter, _ = li.load_data_bananas(batch_size)

device, net = li.try_gpu(), TinySSD(num_classes=1)
...
for epoch in range(num_epochs):
    ...
    for features, target in train_iter:
        ...
        anchors, cls_preds, bbox_preds = net(X)
        # 为每个锚框标注类别和偏移量
        bbox_labels, bbox_masks, cls_labels = li.multibox_target(anchors, Y)
...
# 预测
# net.load_state_dict(torch.load('SSD.params'))
# net = net.to(device)
